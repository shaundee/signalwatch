...
type Status = { found: true; status: string; plan?: string | null } | { found: false } | { error: string };
...
export default function ChangePlanButtons() {
  ...
  async function change(plan: "monthly" | "annual") {
    try {
      ...
      const res = await fetch("/api/change-plan", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ plan }) });
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <button className="px-4 py-2 rounded-2xl border border-zinc-300 hover:bg-zinc-50 disabled:opacity-50"
        onClick={() => change("monthly")} disabled={disabled || busy !== null || currentPlan === "monthly"}>
        {busy === "monthly" ? "Switching…" : "Switch to monthly"}
      </button>
      <button className="px-4 py-2 rounded-2xl border border-zinc-300 hover:bg-zinc-50 disabled:opacity-50"
        onClick={() => change("annual")} disabled={disabled || busy !== null || currentPlan === "annual"}>
